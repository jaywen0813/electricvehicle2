 ...
 * ================================================
 */
public class ZxdLookAndUpdatePresenter extends BasePresenter<ZxdLookAndUpdateContract.View> implements ZxdLookAndUpdateContract.Presenter {
...
//                });
        ...
        NetInstance.getEventsService().zxdlist(ParameterUtils.getHeaser(paramsMap), ParameterUtils.getJsonBody(paramsMap)).
                subscribeOn(Schedulers.io()).
                observeOn(AndroidSchedulers.mainThread()).
                subscribe(new Observer<ZXDListVO>() {
                    @Override
                    public void onSubscribe(Disposable d) {
                        addDisposable(d);
                    }
                    ...
                    @Override
                    public void onNext(ZXDListVO vDate) {
                        if (vDate.getSuccess().equals("T")) {
                            if ((vDate.getData().getDataList() != null) && vDate.getData().getDataList().size()>0) {
