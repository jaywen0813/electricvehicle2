...
//                        }
    ...
    //修改库位号
    @Override
    public void getKWH(String id,String freeLoc) {
...
//post
        NetInstance.getEventsService().updatekwh(ParameterUtils.getHeaser(paramsMap), ParameterUtils.getJsonBody(paramsMap)).
                subscribeOn(Schedulers.io()).
                observeOn(AndroidSchedulers.mainThread()).
                subscribe(new Observer<YiKuVO>() {
                    @Override
                    public void onSubscribe(Disposable d) {
                        addDisposable(d);
                    }
                    ...
                    @Override
                    public void onNext(YiKuVO vDate) {
                        if (vDate.getSuccess().equals("T")) {//如果是200，则代表退出成功了
                            mView.yikuSuccess();
                        }else {
                            mView.yikuMiss();
